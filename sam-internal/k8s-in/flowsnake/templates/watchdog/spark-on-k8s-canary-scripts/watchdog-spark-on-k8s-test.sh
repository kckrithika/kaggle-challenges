...
if [[ ".jsonnet" == "${1: -8}" ]] ; then
    jsonnet -V imageRegistry=${DOCKER_REGISTRY} -V jenkinsId=${UUID} -V dockerTag=${DOCKER_TAG} -V s3ProxyHost=${S3_PROXY_HOST} -V driverServiceAccount=${DRIVER_SERVICE_ACCOUNT} ${1} -m /strata-test-specs-out
