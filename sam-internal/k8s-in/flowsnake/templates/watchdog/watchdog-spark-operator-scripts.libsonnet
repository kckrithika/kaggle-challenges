...
local configmap_data =
    {
        "check-spark-operator.sh": if std.objectHas(flowsnake_images.feature_flags, "spark_operator_watchdog_kubectl_retries")
            then importstr "spark-on-k8s-canary-scripts/watchdog-spark-on-k8s-with-retry.sh"
