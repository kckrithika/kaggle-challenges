...
def update_reserved_ips(reserved_ips_file_path, cluster, new_ip):
    ...
    if cluster_reserved_ips_search_results is None:
        raise Exception('{} was not found in {}'.format(cluster, PUBLIC_RESERVED_IPS_FIELD_NAME))
    ...
    if new_ip in cluster_reserved_ips:
        ...
        with open(reserved_ips_file_path, 'w') as jsonnet_file:
            jsonnet_file.write(reserved_ip_text.replace(public_reserved_ips_text, new_public_reserved_ip_text))

# Input: reserved-ip jsonnet file path, cluster name, new IP
update_reserved_ips(sys.argv[1], sys.argv[2], sys.argv[3])
