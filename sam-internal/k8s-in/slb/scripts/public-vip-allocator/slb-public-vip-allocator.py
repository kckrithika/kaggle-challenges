...
def process_vip_files(root_vip_yaml_path, public_reserved_ips, public_subnets, minimum_octet):
    vips_to_delete = []
    vips_to_add = []
    ...
    for vip_meta_data in vips_to_delete:
        delete_public_vip(vip_meta_data.fqdn,
                          vip_meta_data.cluster,
                          public_reserved_ips)

    for vip_meta_data in vips_to_add:
        add_public_vip(vip_meta_data.fqdn,
                              vip_meta_data.cluster,
                              public_reserved_ips,
                              public_subnets,
                              minimum_octet)
...
def delete_public_vip(fqdn, cluster, public_reserved_ips):
...
def add_public_vip(fqdn,
    ...
    print("Added {} to {} with IP {}".format(fqdn, cluster, new_ip))
