...
local use_insecure = (if configs.estate == "prd-sam" || configs.estate == "prd-samdev" || configs.estate == "prd-sdc" then true else false);
...
# to be set up this way.
  somewhat_secure_mostly_read_only:: @'    # Block deletes except for pods
    # One week exception for mesh.  Remove after 3/8/2019
    acl mesh url_reg /api/v1/namespaces/[a-z-]*mesh[a-z-]*/

    acl pod-kind url_reg /api/v1/namespaces/[a-z0-9-]+/pods/
    acl delete-method method DELETE
    http-request deny if delete-method !pod-kind !sam_api !mesh
    ...
    http-request deny if post-method !exec !sam_api !mesh
    ...
    http-request deny if other-methods !sam_api !mesh
',
