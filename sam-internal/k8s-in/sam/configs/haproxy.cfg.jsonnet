...
# to be set up this way.
  somewhat_secure_mostly_read_only:: @'    # Block deletes except for pods
    ...
    # /api/v1/namespaces/sam-system/pods/k8sproxy-b64d8d9cc-bpgks
    acl delete-method method DELETE
    http-request deny if delete-method !pod-kind

    # Block post (create) except for exec
    ...
    # Block all posts that are not exec
    http-request deny if post-method !exec
    # Block exec on sam-system and kube-system / kube-public
    ...
    http-request deny if other-methods
',
...
frontend localhost
    ...
    acl e2e_secret url_reg /api/v1/namespaces/[a-z0-9-]+/secrets/ceph-sec-user-rdi
    http-request deny if secrets !e2e_secret
    http-request deny if secrets_namespace !e2e_secret
