...
if (configs.estate == "prd-samtest" || configs.estate == "prd-samdev") then {
...
} else if samfeatureflags.kubedns then {
    apiVersion: "extensions/v1beta1",
    kind: "Deployment",
    metadata: {
        labels: {
    ...
    spec: {
        progressDeadlineSeconds: 600,
        replicas: (if configs.estate == "prd-samdev" || configs.estate == "prd-samtest" then 1 else if configs.kingdom == "ord" || configs.kingdom == "frf" then 5 else 3),
        revisionHistoryLimit: 2,
        selector: {
            matchLabels: {
                "k8s-app": "kube-dns",
