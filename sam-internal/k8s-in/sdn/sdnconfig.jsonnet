...
local utils = import "util_functions.jsonnet";
{
    ...
    logToStdErrArg: if (kingdom == "prd") then "--logtostderr=true" else "--logtostderr=false",
    alsoLogToStdErrArg: "--alsologtostderr=true",

    # Volume for logs
    sdn_logs_volume: if (kingdom == "prd") then {} else {
        name: "sdnlogs",
        hostPath: {
          path: "/data/logs/sdn",
        },
    ...
    sdn_logs_volume_mount: if (kingdom == "prd") then {} else {
